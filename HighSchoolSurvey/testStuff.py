...
# Drop columns
# data = data.drop(data.iloc[:, 267:316], axis = 1)
...
# print('Number of Missing Values', data['CountOnPolice'].isna().sum())
# data = data[data['CountOnPolice'].notna()]
# print('****After****\nNumber of Missing Values', data['CountOnPolice'].isna().sum())
# print('Numer of instances = %d' %data.shape[0])
# print('Numer of attributes = %d' %data.shape[1])
...
# Function to simplify CountOnPolice col
def simplify_police (row):
    if row['CountOnPolice'] == 1 or row['CountOnPolice'] == 2:
        return '1'
    if row['CountOnPolice'] == 3 or row['CountOnPolice'] == 4:
        return '2'

# Add a column that converts 'CountOnPolice' to only 1 v 2
# With 1 being agree and 2 being disagree
data['belong'] = data.apply(lambda row: simplify_police(row), axis=1)
# print('CountOnPolice col converted to binary:\n', data.CountOnPolice, '\n', data.belong)
...
data['raceBinary'] = data.apply(lambda row: simplify_race(row), axis=1)
# print('Race col converted to binary:\n', data.Race, '\n', data.raceBinary)

print('Value counts for police:\n', data.belong.value_counts())
print('Value counts for race:\n', data.raceBinary.value_counts())

#Create table for Strongly Agree
SA = data[data['CountOnPolice'] == 1]

#Create table for Agree
A = data[data['CountOnPolice'] == 2]

#Create table for Disagree
D = data[data['CountOnPolice'] == 3]

#Create table for Strongly Disagree
SD = data[data['CountOnPolice'] == 4]

    #Strongly Agree Represenation
#Value you are looking for
ySA = SA['belong']
#Dataset with missing value
xSA = SA.drop(['Schoolcode', 'CountOnPolice', 'belong'], axis = 1)
...
yA = A['belong']
#Dataset with missing value
xA = A.drop(['Schoolcode', 'CountOnPolice', 'belong'], axis = 1)
...
#Value you are looking for
yD = D['belong']
#Dataset with missing valuehttps://stackoverflow.com/questions/49992300/how-to-show-graph-in-visual-studio-code-itself
xD = D.drop(['Schoolcode', 'CountOnPolice', 'belong'], axis = 1)
...
ySD = SD['belong']
#Dataset with missing value
xSD = SD.drop(['Schoolcode', 'CountOnPolice', 'belong'], axis = 1)
...
#Create variables 
Y = data['belong']
X = data.drop(['Schoolcode', 'RespondentID','CountOnPolice','belong'], axis = 1)
