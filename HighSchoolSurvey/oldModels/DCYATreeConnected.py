...
data = data.drop(['OtherProblems', 'Homeless2015', 'Rules', 'CloseToPeople', 'FeelSafe', 'TreatedFairly', 'AdultsToTalkTo', 'IBelong'], axis=1)

print(data.shape)
print('First 5 rows of cleaned** data:\n', data.head(), '\n')
print('Numer of instances = %d' %data.shape[0])
print('Numer of attributes = %d' %data.shape[1])
# print('Number of Missing Values', data['Connected'].isna().sum())
# data = data[data['Connected'].notna()]
# print('****After****\nNumber of Missing Values', data['Connected'].isna().sum())
# print('Numer of instances = %d' %data.shape[0])
# print('Numer of attributes = %d' %data.shape[1])
...
# Function to simplify Connected col
def simplify_belong (row):
    if row['Connected'] == 1 or row['Connected'] == 2:
        return '1'
    if row['Connected'] == 3 or row['Connected'] == 4:
        return '2'

# Add a column that converts 'Connected' to only 1 v 2
# With 1 being agree and 2 being disagree
data['belong'] = data.apply(lambda row: simplify_belong(row), axis=1)
# print('Connected col converted to binary:\n', data.belong, '\n', data.Connected)

data.to_csv('checkshit.csv')

#Create table for Strongly Agree
SA = data[data['Connected'] == 1]

#Create table for Agree
A = data[data['Connected'] == 2]

#Create table for Disagree
D = data[data['Connected'] == 3]

#Create table for Strongly Disagree
SD = data[data['Connected'] == 4]

    #Strongly Agree Represenation
#Value you are looking for
ySA = SA['belong']
#Dataset with missing value
xSA = SA.drop(['Schoolcode', 'Connected', 'belong'], axis = 1)
...
yA = A['belong']
#Dataset with missing value
xA = A.drop(['Schoolcode', 'Connected', 'belong'], axis = 1)
...
xD = D.drop(['Schoolcode', 'Connected', 'belong'], axis = 1)
...
ySD = SD['belong']
#Dataset with missing value
xSD = SD.drop(['Schoolcode', 'Connected', 'belong'], axis = 1)
...
#Create variables 
Y = data['belong']
X = data.drop(['Schoolcode', 'RespondentID','Connected','belong'], axis = 1)
